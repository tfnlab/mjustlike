...
//        if (networkData) {
          ...
          const { abi } = require('../abis/MuzicBox.json');

          var smart_contract_interface = new web3.eth.Contract(abi, '0xAC1e192A284D54F442D97BEe44256eEf7425d7A1')
...
claimPunk = async (punkIndex) => {
  ...
  if (totalSupply > 700){
    const price = window.web3.utils.toWei("0.08", "Ether") * punkIndex;
    this.setState({ loading: true });
      this.state.cryptoBoysContract.methods
        .paidMint(punkIndex)
          ...
          <>
            <HashRouter basename="/">
              ...
              <Route path='/nftrade' component={() => {
                   window.location.href = 'https://opensea.io/collection/muzicbox';
                   return null;
              }}/>
